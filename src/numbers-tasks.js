...
function getAverage(value1, value2) {
  return (
    ((value1 / Number.MAX_VALUE + value2 / Number.MAX_VALUE) / 2) *
    Number.MAX_VALUE
  );
}
 ...
 *
 * @pavalue22 = value2 - Number.MAX_VALUE;= value2 - Number.MAX_VALUE;= value2 - Number.MAX_VALUE;ber} x1
 * @param {number} y1
 * @param {number} x2
 * @param {number} y2
 *
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorLengths1 = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorLengths2 = Math.sqrt(x2 * x2 + y2 * y2);
  const vectorLengthsProduct = vectorLengths1 * vectorLengths2;
  return Math.acos(scalarProduct / vectorLengthsProduct);
}
...
function toNumber(value, def) {
  return Number.isFinite(parseInt(value, 10)) ? value : def;
}
...
function getSumOfDigits(num) {
  ...
  const countNumber = stringNumber.length;
  let sum = 0;
